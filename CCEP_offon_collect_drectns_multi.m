function CCEP_offon_collect_drectns_multi(Sbj_Metadatas)
% CCEP_offon_collect_drectns_multi: to collect and see what changes in what
% percentages in CCEP N1 and N2 latencies and amplitudes in which direction
% based on stimulation and recording electrode character. This version
% includes the directions of how they change as well.
...
for c = 1:length(analyzed_chans)
    ...
    peak_info.N1_time_dir(nanmean(cell2mat(peak_info.off_N1_time_all),2) < nanmean(cell2mat(peak_info.on_N1_time_all),2)) = 1;
    peak_info.N1_time_dir(nanmean(cell2mat(peak_info.off_N1_time_all),2) > nanmean(cell2mat(peak_info.on_N1_time_all),2)) = 2;
...
print(fullfile(res_dir,[comb_sbjIDs '_offon_dir_N1-latency.png']),'-dpng','-r300')
...
print(fullfile(res_dir,[comb_sbjIDs '_offon_dir_N1-amplitude.png']),'-dpng','-r300')
...
print(fullfile(res_dir,[comb_sbjIDs '_offon_dir_N2-latency.png']),'-dpng','-r300')
...
print(fullfile(res_dir,[comb_sbjIDs '_offon_dir_N2-amplitude.png']),'-dpng','-r300')
...
save(fullfile(res_dir,[comb_sbjIDs '_offon_allDirections.mat']),'collected','bp_good_chans_classes_all','bp_good_chans_all','analyzed_chans_character_all','analyzed_chans_all')
