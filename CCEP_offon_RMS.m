function CCEP_offon_RMS(Sbj_Metadata, offmed_block, onmed_block)
...
for i1 = 1:4
    for i2 = 1:4
        ...
        ttestresults.([for_crisscross{i1} '2' for_crisscross{i2}]).dir=nanmean(collected.(['on_' for_crisscross{i1} '2' for_crisscross{i2} 'rms'])-...
            ...
            collected.(['off_' for_crisscross{i1} '2' for_crisscross{i2} 'rms'])));
        if ~isempty(p)
            p_rms(i1,i2)=p;
            if significant
...
% save directory
res_dir=fullfile(Sbj_Metadata.results,'offon_RMS');
if ~isfolder(res_dir); mkdir(res_dir); end

print(fullfile(res_dir,[Sbj_Metadata.sbj_ID '_offon_RMS.png']),'-dpng','-r300')

%% save the data as well

save(fullfile(res_dir,[Sbj_Metadata.sbj_ID '_offon_RMS.mat']),'collected','ttestresults','ttestresultsN1','ttestresultsN2','p_rms','p_rmsN2','p_rmsN1','h_rms','h_rmsN2','h_rmsN1')
